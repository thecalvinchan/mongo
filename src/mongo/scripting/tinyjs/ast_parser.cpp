...
std::unique_ptr<Node> ASTParser::arrayIndexedAction() {
    ...
    if ((child = acceptIf(TokenType::kIdentifier))) {
        ...
        std::unique_ptr<Node> child2;
        if ((child2 = acceptIf(TokenType::kIntegerLiteral)) || (child2 = acceptIf(TokenType::kIdentifier)) ||
...
std::unique_ptr<Node> ASTParser::comparisonOperationAction() {
    ...
    std::unique_ptr<Node> child;
    if ((child = acceptIf(TokenType::kTripleEquals)) || (child = acceptIf(TokenType::kDoubleEquals)) ||
        (child = acceptIf(TokenType::kGreaterThan)) || (child = acceptIf(TokenType::kGreaterThanEquals)) ||
        (child = acceptIf(TokenType::kLessThan)) || (child = acceptIf(TokenType::kLessThanEquals)) ||
        (child = acceptIf(TokenType::kNotEquals)) || (child = acceptIf(TokenType::kDoubleNotEquals))) {
        head->addChild(std::move(child));
    } else {
